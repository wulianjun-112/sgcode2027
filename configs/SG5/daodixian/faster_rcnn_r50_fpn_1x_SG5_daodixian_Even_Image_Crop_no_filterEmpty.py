...
data = dict(
    samples_per_gpu=4,
    workers_per_gpu=4,
    class_aware_sampler=dict(num_sample_class=[2,2]),
    train=dict(
        # type='RepeatDataset',
        # times=3,
        # dataset=dict(
            type=dataset_type,
            ann_file='/data/wulianjun/datasets/SG5/{}/ImageSets/Main/train_crop_no_fliter.txt'.format(sub_dataset_name),
    ...
    val=dict(
        type=dataset_type,
        ann_file='/data/wulianjun/datasets/SG5/{}/ImageSets/Main/val_crop_split.txt'.format(sub_dataset_name),
        img_prefix=val_data_root,
        pipeline=val_pipeline,
        classes=train_class,
        val_classes=val_classes,
        sub_ann='Annotations_crop',
        sub_jpg='JPEGImages_crop'),
    test=dict(
        type=dataset_type,
        ann_file='/data/wulianjun/datasets/SG5/{}/ImageSets/Main/val_crop_split.txt'.format(sub_dataset_name),
        img_prefix=val_data_root,
...
optimizer = dict(type='SGD', lr=0.04/16*4, momentum=0.9, weight_decay=0.0001)
