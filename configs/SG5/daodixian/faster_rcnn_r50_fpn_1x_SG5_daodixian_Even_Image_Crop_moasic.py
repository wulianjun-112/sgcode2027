...
model = dict(
    roi_head=dict(
     ...
     train_cfg=dict(moasic=dict(k=4))
     )
...
train_pipeline = [
    ...
    dict(
        ...
        min_crop_size=0.3),
    dict(type='Resize', img_scale=(672,416), keep_ratio=False),
...
data = dict(
    ...
    train=dict(
        ...
        # dataset=dict(
            type=dataset_type,
            ann_file='/data/wulianjun/datasets/SG5/{}/ImageSets/Main/train_crop_split.txt'.format(sub_dataset_name),
...
optimizer = dict(type='SGD', lr=0.04/16*8, momentum=0.9, weight_decay=0.0001)
...
lr_config = dict(
    ...
    warmup_ratio=0.001,
    step=[4*4,7*4])
runner = dict(type='EpochBasedRunner', max_epochs=9*4)
load_from = None
