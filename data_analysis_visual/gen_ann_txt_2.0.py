...
def read_xml(xml_path,dataset_class,relabel=False,save_path=None,index=0,val=False,ref_object=None):
    ...
    if not relabel:
        ...
        for objects in root.findall('object'):
            ...
            #for daodixian
            if label == '020000111' or label == '020001061' or label == '020100051' \
            ...
            if  label == '020000021 导线 本体 损伤' or label == '020001021 导线 引流线 损伤' \
                or label=='020000021' or label =='020001021' or label == '020100021':
                label_ = '线损伤'

            if  label == '020001031' or label == '020000031' or label == '020000031 导线 本体 松股'   or label=='020001031 导线 引流线 松股' :
                label_ = '线松股'
            
            if label == '020001011'  or label == '020000011' or label == '020100011' \
                  or label =='020000011 导线 本体 断股' :
                label_ = '线断股'
        ...
        if os.path.exists(osp.join(save_path.replace('Annotations_crop_no_fliter','JPEGImages'),Split(file_name)+'.jpg')):
            
            # if os.path.exists(save_path):
            #     save_path = save_path.replace('Annotations','Annotations_')
...
sub_data_name = ['导地线']
data_roots = ['/data/wulianjun/datasets/SG5/{}/Annotations_crop_no_fliter'.format(x) for x in sub_data_name]
save_data_roots = ['/data/wulianjun/datasets/SG5/{}/Annotations_crop_no_fliter'.format(x) for x in sub_data_name]
